...
$fromPopupWindow = $_REQUEST['fromPopupWindow'] ?? false;
$clockonoff = $_REQUEST['clockonoff'] ?? '';
$clock_on_time_hour = $_REQUEST['clock_on_time_hour'] ?? 0;
$clock_on_time_min = $_REQUEST['clock_on_time_min'] ?? 0;
$clock_off_time_hour = $_REQUEST['clock_off_time_hour'] ?? 0;
$clock_off_time_min = $_REQUEST['clock_off_time_min'] ?? 0;
$log_message = $_REQUEST['log_message'] ?? '';
$log_message_presented = $_REQUEST['log_message_presented'] ?? false;
$clock_on_check = $_REQUEST['clock_on_check'] ?? '';
$clock_off_check = $_REQUEST['clock_off_check'] ?? '';
$clock_on_radio = $_REQUEST['clock_on_radio'] ?? '';
$clock_off_radio = $_REQUEST['clock_off_radio'] ?? '';

//set the return location
$Location = "$destination?month=$month&year=$year&day=$day&destination=$destination";
if ('stopwatch.php' == $destination || 'daily.php' == $destination) {
...
if (empty($clockonoff)) {
    ...
    } elseif (!empty($clock_on_check)) {
        if ('now' == $clock_on_radio) {
            $clockonoff = 'clockonnow';
        } else {
            $clockonoff = 'clockonat';
        }
    } elseif (!empty($clock_off_check)) {
        if ('now' == $clock_off_radio) {
...
if ('clockonandoff' == $clockonoff) {
    ...
    function getLogMessage()
    {
        ...
        if (false === $log_message_presented) {
    ...
    function clockonandoff()
    {
        ...
        if ('now' == $clock_on_radio && 'now' == $clock_off_radio) {
        ...
        } elseif ($clock_on_time_hour > $clock_off_time_hour) {
            errorPage("You cannot have your clock on time ($clock_on_time_hour:$clock_on_time_min) " .
                                    "later than your clock off time ($clock_off_time_hour:$clock_off_time_min)", $fromPopupWindow);
        } elseif (($clock_on_time_hour == $clock_off_time_hour) && ($clock_on_time_min == $clock_off_time_min)) {
