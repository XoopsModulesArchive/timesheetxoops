...
if ($mode == 'weekly') {
	$query = "SELECT $TIMES_TABLE.proj_id, ".
				"$TIMES_TABLE.task_id, ".
				"$TIMES_TABLE.log_message, " . 'sec_to_time(unix_timestamp(end_time) - unix_timestamp(start_time)) as diff_time, ' . '(unix_timestamp(end_time) - unix_timestamp(start_time)) as diff, ' .
             "$PROJECT_TABLE.title, ".
				"$TASK_TABLE.name, ".
				"date_format(start_time, '%Y/%m/%d') as start_date, ". 'trans_num ' .
             "FROM $USER_TABLE, $TIMES_TABLE, $PROJECT_TABLE, $TASK_TABLE ".
			"WHERE $TIMES_TABLE.uid=$USER_TABLE.username AND ". 'end_time > 0 AND ' .
             "$TIMES_TABLE.uid='$uid' ".
				"AND start_time >= '$year-$month-$day' AND ".
				"$PROJECT_TABLE.proj_id = $TIMES_TABLE.proj_id AND ".
				"$TASK_TABLE.task_id = $TIMES_TABLE.task_id AND ".
				"end_time < '".date('Y-m-d',$next_week)."' ". 'ORDER BY proj_id, task_id, start_time';
} else {
	$query = "SELECT $TIMES_TABLE.proj_id, ".
				"$TIMES_TABLE.task_id, ".
				"$TIMES_TABLE.log_message, " . 'sec_to_time(unix_timestamp(end_time) - unix_timestamp(start_time)) as diff_time, ' . '(unix_timestamp(end_time) - unix_timestamp(start_time)) as diff, ' .
			...
			"WHERE $TIMES_TABLE.uid=$USER_TABLE.username AND ". 'end_time > 0 AND ' .
             "$TIMES_TABLE.uid='$uid' ".
				...
				"$PROJECT_TABLE.proj_id = $TIMES_TABLE.proj_id AND ".
				"$TASK_TABLE.task_id = $TIMES_TABLE.task_id AND ".
				"end_time < '".date('Y-m-1',$next_month)."' ". 'ORDER BY proj_id, task_id, start_time';
}
//run the query  
					...
					<tr>
						<td align="left" nowrap>
							<table width="100%" height="100%" border="0" cellpadding="1" cellspacing="2">
								<tr>
									<td align="right" width="0" class="outer_table_heading"><?echo _TSX_REPORT_FIELD_USER ?>:</td>
