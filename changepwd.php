...
if (empty($errormsg) && !empty($old_pass)) {
    $qh = $GLOBALS['xoopsDB']->queryF("select password, $DATABASE_PASSWORD_FUNCTION('$old_pass') from $USER_TABLE where username='$contextUser'") || die('Unable to select ' . $GLOBALS['xoopsDB']->error());

    [$check1, $check2] = $GLOBALS['xoopsDB']->fetchRow($qh);

    if ($check1 != $check2) {
        $errormsg = 'Wrong password, sorry!';
    } else {
        $qh = $GLOBALS['xoopsDB']->queryF("update $USER_TABLE set password=$DATABASE_PASSWORD_FUNCTION('$passwd1') where username='$contextUser'");
