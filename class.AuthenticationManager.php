...
if (file_exists('xoops_header.php')) {
    require_once 'xoops_header.php';
}
...
class AuthenticationManager
{
    ...
    public $errorCode = AUTH_NONE;

    /**
     * The error text
     */

    public $errorText = 'Authentication has not yet been attempted';
    ...
    public $ldapServerErrorCode;

    /**
     * The error description returned from the LDAP server
     */

    public $ldapServerErrorText;
    ...
    */
    ...
    public function login($username, $password)
    {
        ...
        if ($this->usingLDAP()) {
            ...
            [$qh, $num] = dbQuery(
                "SELECT password AS passwd1, $DATABASE_PASSWORD_FUNCTION('$password') AS passwd2 " . "FROM $USER_TABLE WHERE username='$username'"
            );
...
//			}
        ...
        [$qh, $num] = dbQuery(
            'SELECT level ' . "FROM $USER_TABLE WHERE username='$username'"
        );
    ...
    public function hasClearance($accessLevel)
    {
        //start/continue the session
    ...
    */
    ...
    public function usingLDAP()
    {
        require 'table_names.inc';

        [$qh, $num] = dbQuery("SELECT useLDAP FROM $CONFIG_TABLE WHERE config_set_id='1'");

        $data = dbResult($qh);

        return 1 == $data['useLDAP'];
    }
    ...
    public function ldapLogin($username, $password)
    {
        ...
        [$qh, $num] = dbQuery(
            'SELECT LDAPScheme, LDAPHost, LDAPPort, LDAPBaseDN, ' . 'LDAPUsernameAttribute, LDAPSearchScope, LDAPFilter, LDAPProtocolVersion, ' . 'LDAPBindUsername, LDAPBindPassword ' . "FROM $CONFIG_TABLE WHERE config_set_id='1'"
        );
        ...
        if ('' != $data['LDAPFilter']) {
            ...
            $length = mb_strlen($userFilter);

            if ('(' == $userFilter[0] && ')' == $userFilter[$length - 1]) {
                $userFilter = mb_substr($userFilter, 1, $length - 2);
            }

            $filter = '(&(' . $userFilter . ')(' . $filter . '))';
        }

        if ('base' == $data['LDAPSearchScope']) {
        ...
        } elseif ('one' == $data['LDAPSearchScope']) {
            //search the directory returning records in the base dn
        ...
        if (!isset($attributes['givenName'])) {
            if (isset($attributes['cn'])) {
                $spacePos = mb_strpos($attributes['cn'][0], ' ');

                if (!(false === $spacePos)) {
                    $firstName = mb_substr($attributes['cn'][0], 0, $spacePos);
                } else {
        ...
        if (!$this->userExists($username)) {
            ...
            [$qh, $num] = dbQuery(
                'SELECT first_name, last_name, email_address, phone, bill_rate ' . "FROM $USER_TABLE WHERE username='$username'"
            );
            ...
            if (0 != $existingUserDetails['bill_rate']) {
    ...
    public function userExists($username)
    {
        ...
        [$qh, $num] = dbQuery("select username from $USER_TABLE where username='$username'");
    ...
    {
        if (AUTH_FAILED_LDAP_LOGIN != $this->errorCode) {
            return $this->errorText;
        }

        if (LDAP_SERVER_ERROR != $this->ldapErrorCode) {
...
class AuthenticationManagerXoop extends AuthenticationManager
{
    public function login($username, $password)
    {
        $_SESSION['loggedInUser'] = $username;
    ...
    public function isLoggedIn($arg = null)
    {
        if (null === $arg) {
            //Then call usual isLoggedIn method

            return AuthenticationManager::isLoggedIn();
        }
  

        return $this->isInAdminGroup($arg);
    }

    // Validate if user has access to admin function - Based on Groups in Xoops

    public function isInAdminGroup($adminType)
    {
        ...
        if (is_object($xoopsUser)) {
            foreach ($xoopsUser->getGroups() as $group) {
                if (in_array($group, $xoopsModuleConfig[$adminType], true) || XOOPS_GROUP_ADMIN == $group) {
                    $allowed = true;
...
$authenticationManager = new AuthenticationManagerXoop();
