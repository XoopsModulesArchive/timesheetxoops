...
if (!isset($action)) {
    ...
    if (!checkdate($end_month, $end_day, $end_year) || !checkdate($start_month, $start_day, $start_year)) {
        if ((0 != $start_day && 0 != $start_month && 0 != $start_year) || (0 != $end_day && 0 != $end_month && 0 != $end_year)) {
    ...
    [$qh, $num] = dbQuery(
        "INSERT INTO $PROJECT_TABLE (title, client_id, description, start_date, deadline, http_link, proj_status, proj_leader) VALUES " . "('$title','$client_id','$description', '$start_year-$start_month-$start_day', " . "'$end_year-$end_month-$end_day','$url', '$proj_status', '$project_leader')"
    );
    ...
    [$task_qh, $num] = dbQuery(
        "INSERT INTO $TASK_TABLE (proj_id, name, description, assigned, started, status)\n VALUES " . "($proj_id, 'Default Task', '', '$time_string', '$time_string', 'Started')"
    );
