...
if ($proj_id != 0) { // id 0 means 'All Projects'
	[$qh, $num] = dbQuery("SELECT * FROM $ASSIGNMENTS_TABLE where proj_id='$proj_id' and username='$contextUser'");
	if ($num < 1)
...
//get the timeformat
[$qh2, $numq] = dbQuery("select timeformat from $CONFIG_TABLE where config_set_id = '1'");
...
$query .= 'unix_timestamp(end_time) - unix_timestamp(start_time) AS diff_sec, ' . 'end_time AS end_time_str, ' . 'start_time AS start_time_str, ' . 'unix_timestamp(start_time) AS start_time, ' . 'unix_timestamp(end_time) AS end_time, ' .
...
$query .= "$TASK_TABLE.task_id = $TIMES_TABLE.task_id AND ".
			...
			"OR (start_time < '$startYear-$startMonth-$startDay 00:00:00' AND end_time >= '$endYear-$endMonth-$endDay 00:00:00')) ". 'ORDER BY proj_id, start_time';

?>
<!-- <html> -->
<!-- <head> -->

<title>Simple Weekly Timesheet for <?php echo (string)$contextUser ?></title>
<?php
include ('header.inc');
?>
...
[$qh3, $num3] = dbQuery($getProjectsQuery);
...
[$qh4, $num4] = dbQuery($getTasksQuery);
//iterate through results
for ($i = 0; $i < $num4; $i++) {
	//get the current record
	$data = dbResult($qh4, $i);
				...
				<table width="100%" border="0">
					<tr>
						<td align="left" nowrap class="outer_table_heading">
							<?echo _TSX_TS_WKSTART ?>: <?php echo date('D F j, Y', mktime(0, 0, 0, $startMonth, $startDay, $startYear)); ?>
						</td>
						<td align="right" nowrap>
							<a href="<?php echo $_SERVER['PHP_SELF']; ?>?proj_id=<?echo $proj_id; ?>&task_id=<?php echo $task_id; ?>&year=<?echo $previousWeekYear ?>&month=<?php echo $previousWeekMonth ?>&day=<?php echo $previousWeekDay ?>" class="outer_table_action"><?echo _TSX_GEN_PREV ?></a>
							<a href="<?php echo $_SERVER['PHP_SELF']; ?>?proj_id=<?php echo $proj_id; ?>&task_id=<?php echo $task_id; ?>&year=<?php echo $nextWeekYear ?>&month=<?php echo $nextWeekMonth ?>&day=<?php echo $nextWeekDay ?>" class="outer_table_action"><?echo _TSX_GEN_NEXT ?></a>
						</td>
						<td align="right" nowrap>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_body">
					<tr class="inner_table_head">
						...
						for ($i = 0; $i < 7; $i++) {
							...
							print '<td align="center" width="65">' . '<table width="65" border="0" cellspacing="0" cellpadding="0"><tr>' . '<td class="inner_table_column_heading" align="center">' . (string)$currentDayStr .
                                  "</td></tr></table></td>\n";
						}
						?>
						<td align="center" width="2">&nbsp;</td>
...
<?php 
	...
	//print "<p>WEEK start: $startDateStr WEEK end: $endDateStr</p>";
                    ...
                    class TaskInfo extends Pair
                    {
                        ...
                        public function __construct($value1, $value2, $projectId, $projectTitle, $taskName)
                        {
	...
	//set vars
	$previousProjectId = -1;
	$allTasksDayTotals = [0,0,0,0,0,0,0]; //totals for each day
...
/*	$previousTaskId = -1;	
	...
	?><tr id="row<?php echo $i; ?>"><?php
		?><td class="calendar_cell_middle" valign="top"><?php
			?><table width="100%" border="0" cellspacing="0" cellpadding="0"><?php
				?><tr><?php
						?><td width="50%" align="left"><?php
							?><input type="hidden" id="project_row<?php echo $i; ?>" name="project_row<?php echo $i; ?>" value="<?php echo $matchedPair->projectId; ?>"><?php
							?><select id="projectSelect_row<?php echo $i; ?>" name="projectSelect_row<?php echo $i; ?>" onChange="onChangeProjectSelect(this.id);" style="width: 100%;"><?php
						?></td><?php
						?><td width="50%" align="left"><?php
							?><input type="hidden" id="task_row<?php echo $i; ?>" name="task_row<?php echo $i; ?>" value="<?php echo $matchedPair->value1; ?>"><?php
							?><select id="taskSelect_row<?php echo $i; ?>" name="taskSelect_row<?php echo $i; ?>" onChange="onChangeTaskSelect(this.id);" style="width: 100%;"><?php
						?></td><?php
				?></tr><?php
				?><tr><?php
						?><td height="1"><img src="images/spacer.gif" width="100" height="1"></td><?php
				?></tr><?php
			?></table><?php
		?></td><?php
	...
	/////////////////////////////////////////
	?><tr id="row<?php echo $count; ?>"><?php
		?><td class="calendar_cell_middle" valign="top"><?php
			?><table width="100%" border="0" cellspacing="0" cellpadding="0"><?php
				?><tr><?php
						?><td width="50%" align="left"><?php
							?><input type="hidden" id="project_row<?php echo $count; ?>" name="project_row<?php echo $count; ?>" value="-1"><?php
							?><select id="projectSelect_row<?php echo $count; ?>" name="projectSelect_row<?php echo $count; ?>" onChange="onChangeProjectSelect(this.id);" style="width: 100%;"><?php
						?></td><?php
						?><td width="50%" align="left"><?php
							?><input type="hidden" id="task_row<?php echo $count; ?>" name="task_row<?php echo $count; ?>" value="-1"><?php
							?><select id="taskSelect_row<?php echo $count; ?>" name="taskSelect_row<?php echo $count; ?>" onChange="onChangeTaskSelect(this.id);" style="width: 100%;"><?php
						?></td><?php
				?></tr><?php
				?><tr><?php
						?><td height="1"><img src="images/spacer.gif" width="100" height="1"></td><?php
...
//	for ($i=0; $i<10; $i
	...
	print '<td class="calendar_cell_disabled_middle" align="right">' . _TSX_TS_TOTALHRS . ":</td>\n";
	print "<td class=\"calendar_cell_disabled_middle\" width=\"2\">&nbsp;</td>\n";
